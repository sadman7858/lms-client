...
function RequestedBookStudent() {
  ...
  const handleCancel = (id) => {
    axios
      .put(`http://localhost:4000/studentauth/returnpendingrequestcancel/${id}`)
      .then((res) => {
        if (res.data.status) {
          toast('Book Request Cancel Now!', { position: 'top-left' });
          navigate('/student/dashbord/');
        } else {
          toast('Err in fetch data,returnpendingrequest cancel!', {
            position: 'top-left',
          });
        }
      })
